...
class HermiteSpline {
    ...
    // Compute the position on the spline at time t (0 <= t <= 1)
    computePosition(t) {
      let t2 = t * t;
      let t3 = t2 * t;
  
      let p = this.P0.times(2 * t3 - 3 * t2 + 1); // Position terms for P0
      let v = this.V0.times(t3 - 2 * t2 + t); // Velocity terms for V0
      let q = this.P1.times(-2 * t3 + 3 * t2); // Position terms for P1
      let w = this.V1.times(t3 - t2); // Velocity terms for V1
  
      return p.plus(v).plus(q).plus(w);
    }
  }


  export class DetachedSeed {
    constructor(startPos, startVel) {
      this.position = startPos; // Starting position
      this.velocity = startVel; // Starting velocity
      this.time = 0; // Time progresses from 0 to 1 (or beyond for continuous motion)
      this.hermiteSpline = new HermiteSpline(startPos, startVel, startPos.plus(vec3(0, 0, 10)), vec3(0, 0, 0)); 
    }
    ...
    updatePosition(dt) {
      ...
      if (this.time > 1) {
        ...
        this.position = this.hermiteSpline.computePosition(this.time);
    }
  
    draw(webgl_manager, uniforms, material) {
      const transform = Mat4.translation(this.position[0], this.position[1], this.position[2]);
      this.shapes.sphere.draw(webgl_manager, uniforms, transform, material);
    }
  }
    ...
    const Dandelion =
        class Dandelion {
            constructor(ground_pos, detach_enabled = true) {
                ...
                this.detached_seeds = []; // Store detached seeds
            ...
            detachSeed() {
                const startVel = vec3(Math.random() * 2 - 1, Math.random() * 2 - 1, Math.random() * 2 - 1); // Random velocity
                const newSeed = new DetachedSeed(this.position, startVel);
                this.detached_seeds.push(newSeed); // Add the seed to the list of detached seeds
              }
            ...
            update(dt, active_wind_fields) {
                ...
                for (let seed of this.detached_seeds) {
                    seed.updatePosition(dt);
                  }
            }
            ...
            applySeedForces(dt, wind_field) {
                ...
                for (let i = 0; i < this.seeds.length; i++) {
                    ...
                    if (seed.detached) {
                        detached_seeds.push(new DetachedSeed(seed.get_end_effector_global_position(), seed.wind_direction));
                        continue;
                    }
